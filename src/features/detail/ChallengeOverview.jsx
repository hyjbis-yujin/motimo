...
/**
 * 챌린지 상세 - 참여 인원 요약 (아바타 스택 + 텍스트)
 */
export function ParticipantSummaryPill({ total }) {
  return (
    <div className="flex flex-wrap items-center justify-center bg-primary-light min-h-header rounded-pill px-6 py-3 gap-3 self-center">
      {/* 아바타 스택 */}
      <div className="flex -space-x-3 flex-shrink-0">
        {STATIC_AVATARS.map((src, i) => (
          <div 
            key={i}
            className="w-10 h-10 rounded-full bg-white border-2 border-primary-light flex items-center justify-center overflow-hidden shadow-sm"
          >
            <img 
              src={src} 
              alt="Participant" 
      ...
      {/* 텍스트 정보 */}
      <p className="text-text-dark text-base font-medium leading-tight">
...
export default function ChallengeOverview({ title, description, participants, badge }) {
  return (
    <div className="flex flex-col text-center px-layout-x gap-3 mb-6">
      ...
      <h2 className="text-xl font-bold text-text-dark leading-tight">
        {title}
      </h2>
